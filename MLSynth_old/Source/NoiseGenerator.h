...
class NoiseGenerator {
  public:
    ...
    static void whiteNoiseGenerator(juce::AudioBuffer<float> &buffer,
        ...
        for (int channel = 0; channel < buffer.getNumChannels(); ++channel) {
            ...
            for (int sample = 0; sample < numSamples; ++sample)
                writePtr[sample] =
                    (random.nextFloat() * 2.0f - 1.0f) * amplitude;
        }
    }
    ...
    static void pinkNoiseGenerator(juce::AudioBuffer<float> &buffer,
        ...
        // Filter state (should persist if used continuously — here it's per
        // call)
        ...
        for (int channel = 0; channel < buffer.getNumChannels(); ++channel) {
            ...
            for (int i = 0; i < numSamples; ++i) {
                ...
                writePtr[i] =
                    pink * 0.5f * amplitude; // scaled to avoid clipping
            }
        }
    }
};
