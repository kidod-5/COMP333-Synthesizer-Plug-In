...
void printModelInfo(const torch::jit::script::Module& model, int max_layers = 5) {
    ...
    std::cout << "Approximate model size in memory: " << sizeMB << " MB" << std::endl;
}


torch::jit::script::Module loadTorchScriptModelAsync(const std::string& model_path) {
//    std::thread([this, model_path]() {
//        try {
//            auto newModel = std::make_shared<torch::jit::script::Module>(torch::jit::load(model_path));
//            current_Model.store(newModel);
//            std::cout << "Module loaded and swapped: " << model_path << std::endl;
//        } catch (...) {
//            std::cerr << "FAILED loading model: " << model_path << std::endl;
//        }
//    }).detach();
//}
...
torch::Tensor audioBufferToTensor(const juce::AudioBuffer<float>& buffer) {
    ...
    torch::Tensor inputTensor = torch::zeros({1, 1, raveBlock}, torch::kFloat32);
    ...
    std::memcpy(inputTensor[0][0].data_ptr<float>(),
                in,
...
void tensorToAudioBuffer(const torch::Tensor& tensor, juce::AudioBuffer<float>& buffer) {
    const int numSamples = buffer.getNumSamples();
